...
int main() {
    ...
    // Using simple builder pattern
    auto wgs_result = convert(local_point).withDatum(seattle_datum).asWGS();
    ...
    auto enu_result = convert(portland).withDatum(seattle_datum).asENU();

    std::cout << "   ENU Result: (" << enu_result.x << ", " << enu_result.y << ", " << enu_result.z << ")" << std::endl;

    // Example 3: Fluent chaining with multiple transformations
    std::cout << "\n3. Multiple transformations in one chain:" << std::endl;

    // This would be the ideal syntax:
    // auto multi_result = coordinate(local_point)
    //     .as<ENU>(seattle_datum)
    //     .to<WGS>()
    //     .to<UTM>(10)  // UTM Zone 10
    //     .build();

    std::cout << "   Multi-step transformations are supported through the builder pattern" << std::endl;
    ...
    WGS vancouver(49.2827, -123.1207, 70.0);

    // Convert both to ENU for distance calculation
    auto seattle_enu = convert(seattle).withDatum(seattle_datum).asENU();

    auto vancouver_enu = convert(vancouver).withDatum(seattle_datum).asENU();
