...
void test_rtk_precision() {
    ...
    for (size_t i = 0; i < test_points.size(); i += 2) {
        ...
        double expected_distance = std::sqrt(lat_diff_m*lat_diff_m + lon_diff_m*lon_diff_m + alt_diff_m*alt_diff_m);
...
void test_transformation_consistency() {
    ...
    auto [x2, y2, z2] = ecef_to_enu(std::make_tuple(x_ecef, y_ecef, z_ecef), 
                                    std::make_tuple(lat_ref, lon_ref, alt_ref));
    ...
    std::cout << "Consistency error:  " << consistency_error << " m (" << consistency_error * 1000 << " mm)" << std::endl;
...
void test_ecef_roundtrip() {
    ...
    for (const auto& [lat, lon, alt] : test_coords) {
        ...
        double total_error_mm = std::sqrt(lat_error_mm*lat_error_mm + lon_error_mm*lon_error_mm + alt_error_mm*alt_error_mm);
...
void test_small_displacements() {
    ...
    for (const auto& [name, dlat, dlon, dalt] : displacements) {
        ...
        std::cout << "  Round-trip error: " << lat_error_mm << ", " << lon_error_mm << ", " << alt_error_mm << " mm" << std::endl;
    }
}
